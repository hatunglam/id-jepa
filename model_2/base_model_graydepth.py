...
from model_2.vit import GrayImageDepthTransformer

class IDJEPA_base(GrayImageDepthTransformer):
    def __init__(
        self,
        decoder_depth: int,
    ...
    def forward_base(
        ...
        if test_mode:
            return x_img, x_dep  # (batch_size, num_patches, embed_dim)
        ...
        context_encoding: torch.Tensor = (
            self.post_enc_norm_jepa(  # NOTE: `context_encoding` contains positional information from `x`, which underwent the `self.forward_vit()` pass
                self.student_encoder(  # student encoder (ViT)
                    x=context_block,
                )
            )
        )  # (batch_size, num_context_patches, embed_dim)
